		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
				...
				form.on('submit(out)', function(data){
					...
					var url = 'Currency/selectCurrencyApprover2.action?currency_type=76&currency_string='+staffid;
					if (date1 != null && date1 != ''){
					...
					$.ajax({
						...
						success: function(res) {
							...
							data.unshift({
								...
								currency_date2: '补卡日期',currency_string2: '时间段',currency_string3:"时间点",currency_string7:"备注"
							});
							// 3. 执行导出函数，系统会弹出弹框
							LAY_EXCEL.exportExcel(data, '补卡统计.xlsx', 'xlsx');
						}
					});
				...
				table.render({
					...
					,cols: [[ //表头
						...
						,{field: 'currency_date2', title: '补卡日期', minWidth:100, sort: true,templet:'<div>{{ Format(d.currency_date2,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string2', title: '时间段', minWidth:100}
						,{field: 'currency_string3', title: '时间点', minWidth:100}
						...
						,{title:'操作',fixed: 'right', toolbar: '#barDemo', minWidth:150}
					]]
				});
					...
					if(layEvent === 'detail'){ //查看
						...
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=76"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '50%'],
							content: 'Currency/approvalOpinion.action?approval_id=76&currency_id='+data.currency_id
						});
					}
				});
