		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectCurrencyApprover2.action?currency_type=24&currency_string='+staffid;
                    if (date1 != null && date1 != ''){
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            data.unshift({
                                ...
                                currency_date2: '加班开始时间',currency_date3: '加班结束时间',currency_string2:"加班时长",currency_string7:"申请事由"
                            });
                            // 3. 执行导出函数，系统会弹出弹框
                            LAY_EXCEL.exportExcel(data, '加班统计.xlsx', 'xlsx');
                        }
                    });
...
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_date2', title: '加班开始时间', minWidth:120, templet:'<div>{{ Format(d.currency_date2,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_date3', title: '加班结束时间', minWidth:120, templet:'<div>{{ Format(d.currency_date3,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string2', title: '加班时长', minWidth:100}
					...
					if(layEvent === 'del'){ //删除
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=24"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '50%'],
							content: 'Currency/approvalOpinion.action?approval_id=24&currency_id='+data.currency_id
						});
					}
				});
