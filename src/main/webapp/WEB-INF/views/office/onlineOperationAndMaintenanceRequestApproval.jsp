...
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_date2', title: '调休开始时间', minWidth:120, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_date3', title: '调休结束时间', minWidth:120, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string2', title: '调休时长', minWidth:100}
					...
					if(layEvent === 'del'){ //删除
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=26"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '40%'],
							content: 'Currency/approvalOpinion.action?approval_id=26&currency_id='+data.currency_id
						});
					}
				});
