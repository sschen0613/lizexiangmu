...
<html>
	...
	<body>
		...
		<script type="text/html" id="barDemo">
			...
			<a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="download">下载附件</a>
		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectCurrencyApprover2.action?currency_type=17&currency_string='+staffid+'&currency_string10='+flag;
                    if (date1 != null && date1 != ''){
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            data.unshift({
                                ...
                                currency_string7: '申请事由',currency_string8: '技术要求',currency_string9: '其他说明',
                                details_string2:"物品名称",details_string6:"规格型号",
                                details_string7:"单位",details_money:"数量",details_money2:"库存数量",details_string8:"使用科室"
                            });
                            // 3. 执行导出函数，系统会弹出弹框
                            LAY_EXCEL.exportExcel(data, '药品申请.xlsx', 'xlsx');
                        }
                    });
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Currency/selectCurrencyApprover.action?currency_type='+ currency_type+"&currency_string="+staffid+'&currency_string10='+flag //数据接口
					,page: true //开启分页
					,toolbar: true
			    	,title: '药品请购审批'
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'department_name', title: '申请部门', minWidth:80}
						...
						,{fixed: 'right', title:'操作', toolbar: '#barDemo', minWidth:380}
					]],
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看
						layer.open({
                            ...
                            content: 'storage/requisitionsOfDrugsSuppliesApprovalDetail.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type
                        });
						...
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=17"//iframe的url
						}); 
					}else if(layEvent === 'download'){ //下载附件
                        window.location.href = "Currency/download.action?currency_id="+data.currency_id;
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '40%'],
							content: 'Currency/approvalOpinion.action?approval_id=17&currency_id='+data.currency_id
						});
                        ...
                        layer.open({
                            ...
                            maxmin: true,
                            area: ['80%', '80%'],
                            content: 'storage/requisitionsOfDrugsSuppliesApprovalPrint.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type
                        });
