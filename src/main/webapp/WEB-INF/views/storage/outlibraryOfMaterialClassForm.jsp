	...
	<body>
		...
		<table class="layui-table table-open">
			...
			<tbody>
				...
				<tr>
					<td>区域</td>
					<td colspan=3><select name="area" id="area" lay-filter="area" lay-search lay-verify="required" disabled></select></td>
					<td>客户名称</td>
			...
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
				...
				//监听提交按钮
              	form.on('submit(submitForm)', function(data){
                    ...
                    var quota = $('#contract_id').find('option:selected').attr('data-quota');//合同限额
        		...
        		form.on('select(customer_name)', function(data){
    				...
    				$.ajax({
    					...
    					success:function(res){
    						...
    						$.each(res.data,function(index,item){
    							html += '<option value="'+item.strContractID+'"  data-moneyBegin="'+item.cDefine14+'" data-name="'+item.strContractName+'" data-money="'+item.dblTotalCurrency+'" data-quota="'+item.cDefine16+'" >名称：'+item.strContractName
