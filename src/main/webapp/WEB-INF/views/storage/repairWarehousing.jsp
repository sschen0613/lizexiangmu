	...
	<body>
		...
		<script type="text/html" id="toolbarDemo">
			<div class="layui-btn-container">
				<button class="layui-btn layui-btn-xs" lay-event="add">条码录入申请</button>
			</div>
		</script>
...
// 				});
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Currency/selectApplicantCurrency.action?currency_type=52' //数据接口
					,page: true //开启分页
					,toolbar: '#toolbarDemo'
			    	,title: '条码录入查询页面'
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_number', title: '单据编号', minWidth:200, sort:true}
						,{field: 'currency_string8', title: '仓库', minWidth:100}
						,{field: 'currency_string7', title: '出库类型', minWidth:100}
                        ,{field: 'currency_string10', title: '货品名称', minWidth:100}
                        ,{field: 'currency_string11', title: '货品编码', minWidth:100}
                        ,{field: 'currency_string12', title: '数量', minWidth:100}
                        ,{field: 'currency_string13', title: '单位', minWidth:100}
                        ,{field: 'currency_string14', title: '规格型号', minWidth:100}
                        ,{field: 'currency_string15', title: '备注', minWidth:100}
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看明细
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
							content: 'storage/repairWarehousingApproval.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type //iframe的url
						}); 
					...
					switch(obj.event){
						case 'add':
							layer.open({
								...
								maxmin: true,
								area: ['80%', '80%'],
								content: 'storage/repairWarehousingForm.action' //iframe的url
							}); 
				    	break;
				    	case 'delete':
				    		layer.msg('删除');
