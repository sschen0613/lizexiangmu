	...
	<body>
	 	<form class="layui-form form-top" action="">
	 		<div class="layui-form-item">
				...
				<div class="layui-inline">
					<label class="layui-form-label label-revise">技术要求:</label>
					<div class="layui-input-block">
						<input type="text" name="currency_string8" id="jishu" placeholder="技术要求 :" autocomplete="off" class="layui-input input-revise">
					</div>
				</div>
			  	<div class="layui-inline">
		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectApplicantCurrency1.action?currency_type=17';
                    if (jishu != null && jishu != ''){
                        url += '&currency_string8='+jishu;
                    }
                    if (date1 != null && date1 != ''){
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            // 3. 执行导出函数，系统会弹出弹框
                            LAY_EXCEL.exportExcel(data, '药品请购.xlsx', 'xlsx');
                        }
                    });
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Currency/selectApplicantCurrency.action?currency_type=17' //数据接口
					,page: true //开启分页
					,toolbar: '#toolbarDemo'
			    	,title: '药品请购申请'
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'staff_name', title: '申请人', minWidth:80, sort: true}
						,{field: 'department_name', title: '申请部门', minWidth:100}
						,{field: 'currency_date', title: '申请日期', minWidth:100,templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string7', title: '申请事由', minWidth:100}
						,{field: 'currency_string8', title: '技术要求', minWidth:100}
						,{field: 'currency_string9', title: '其他说明', minWidth:100}
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看明细
						...
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
							content: 'storage/requisitionsOfDrugsFormRevise.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type //iframe的url
						});
				    	//同步更新缓存对应的值
				   		obj.update({
					    	// username: '123'
					...
					switch(obj.event){
						case 'add':
							layer.open({
								...
								maxmin: true,
								area: ['80%', '80%'],
								content: 'storage/requisitionsOfDrugsForm.action?flag='+flag //iframe的url
							}); 
				    	break;
				    	case 'delete':
				    		layer.msg('删除');
