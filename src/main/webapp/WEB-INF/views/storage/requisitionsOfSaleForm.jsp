		...
		<table class="layui-table table-open">
			<thead>
		 	...
		 	<tbody >
				...
				<tr class="money">
					<td>期初合同已收款</td>
					<td colspan=9><input type="text" id="moneyBegin" name="moneyBegin" readonly></td>
				</tr>
		 		<tr>
		 		...
		 		<tr id='prc'>
		 			...
		 			<td>请购数量</td>
		 			<td>库存数量</td>
					<td name="recent">参考成本</td>
		 			<td name="recent">最低售价</td>
					<td name="recent">销售单价</td>
		 			<td>操作</td>
		 		</tr>
		 		<tr class="details">
					...
					<td name="recent"><input type="text" name="recent_price" readonly></td>
		 			<td name="recent"><input type="text" name="iInvLSCost" readonly></td>
					<td name="recent"><input type="text" name="dbl_price" readonly></td>
				...
				//监听提交按钮
              	form.on('submit(submitForm)', function(data){
					...
					$.each($('.details'),function(index,item){
						...
						var recent_price = Number($(item).find('input[name="recent_price"]').val()); //参考成本
                        var iInvLSCost = Number($(item).find('input[name="iInvLSCost"]').val()); //最低售价
                        var dbl_price = Number($(item).find('input[name="dbl_price"]').val()); //销售单价
...
// 						}
						var obj = {'details_string':number
                            	...
                            	,'details_string10':iInvLSCost
                            	,'details_string9':dbl_price
						};
    				...
    				$.ajax({
    					...
    					success:function(res){
    						...
    						$.each(res.data,function(index,item){
    							...
    							var recentPrice = 0;//参考成本
    							var iInvLSCost = 0;//最低售价
    							//获取最近日期商品价格
    							$.ajax({
                                    ...
                                    url:'System/selectInvSPrice.action',
	    				       		...
	    				       		success:function(res){
                                        ...
                                        if(res.data != null) recentPrice = res.data.iInvSPrice;//最低售价
                                        if(res.data != null) iInvLSCost = res.data.iInvLSCost;//最低售价
	    				       			...
	    				       			if (position_Id != 5 && position_Id != 77 && position_Id != 76 && position_Id != 73 && position_Id != 29 && position_Id != 70){
                                            html+=	'<td name="recent" style="display: none"><input type="text" name="recent_price" value="'+recentPrice+'" readonly></td>'
                                                +	'<td name="recent" style="display: none"><input type="text" name="iInvLSCost" value="'+iInvLSCost+'" readonly></td>'
												+	'<td name="recent" style="display: none"><input type="text" name="dbl_price" value="'+item.含税原币单价+'" readonly></td>'
                                            ...
                                            html+=	'<td name="recent"><input type="text" name="recent_price" value="'+recentPrice+'" readonly></td>'
                                            	+	'<td name="recent"><input type="text" name="iInvLSCost" value="'+iInvLSCost+'" readonly></td>'
												+	'<td name="recent"><input type="text" name="dbl_price" value="'+item.含税原币单价+'" readonly></td>'
                                                +	'<td class="delete'+d_count+'"><button type="button" class="layui-btn layui-btn-danger layui-btn-xs">删除</button></td>'
                                                + '</tr>';
                                        }
