...
<form class="layui-form form-open" action="">
	<table class="layui-table table-open">
		<thead>
		<tr>
			<th colspan="10" class="table-h">设备、配件销售请购单（滨州丽泽）</th>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>期望到货日期</td>
			<td colspan=2><input type="text" name="expectedDelivery_date" id="date1" class="layui-input" placeholder="请选择日期" lay-verify="required"></td>
			<td>预计到货日期</td>
			<td colspan=3><input type="text" name="expectedArrival_date" id="date2" class="layui-input" placeholder="请选择日期" lay-verify="required"></td>
		</tr>
		<tr>
			<td>物资部库管员确认入库</td>
			<td colspan=9>
				<input type="radio" name="confirm_inventory" value="1" title="是" checked>
				<input type="radio" name="confirm_inventory" value="2" title="否">
			</td>
		</tr>
		<tr>
...
// 						}
                var obj = {'details_string':number
                    ...
                    ,'details_date3':expectedArrival_date
...
// 					}
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                cCusDepart = res.data.currency_string11; //保存业务员部门
                // 过程一第一级 - 获取所有区域
                $.ajax({
                    url:'System/getRegion.action',
                    type:'post',
                        ...
                        $.ajax({
                            ...
                            success:function(res){
                                ...
                                $.ajax({
                                    url:'System/selectContract.action',
                                    type:'post',
                                    data:{"cCusCode": customerId,"strContractType":"01"},
                                    dataType:'JSON',
                                    success:function(res){
                                        var html = '<option value="">请选择销售合同编号</option>';
                                        $.each(res.data,function(index,item){
                                        ...
                                        $.ajax({
                                            ...
                                            success:function(res){
                                                ...
                                                $.ajax({
                                                    ...
                                                    success:function (res) {
                                                        $.each(res.data,function(index,item){
                                                            $('#buy_quantity'+(index+2)).val(item.details_money);
                                                            if(flag0 == 0){$('#buy_quantity'+(index+2)).attr('readonly','readonly');}//数量禁止修改
                                                        });
                                                    }
                                                });
                                            }
                ...
                $('input[name="expectedDelivery_date"]').val(Format0(res.data.currency_date2,"yyyy-MM-dd HH:mm:ss"));//期望到货日期
                $('input[name="expectedArrival_date"]').val(Format0(res.data.currency_date3,"yyyy-MM-dd HH:mm:ss"));//预计到货日期
                $($('input[name="confirm_inventory"]')[0]).val() == res.data.currency_int2 ? 1 : function(){
                    $('input[name="confirm_inventory"]').removeClass('checked');
                    $($('input[name="confirm_inventory"]')[1]).attr('checked','checked');
                    //form.render('radio');
                };//物资部库管员确认入库
                $($('input[name="delivery_type"]')[0]).val() == res.data.currency_int4 ? 1 : function(){
                    $('input[name="delivery_type"]').removeClass('checked');
                    $($('input[name="delivery_type"]')[1]).attr('checked','checked');
                    //form.render('radio');
                };//发货类型
                ...
                if(res.msg!=null){//禁止修改
                    ...
                    $('input[name="expectedArrival_date"]').attr('disabled','disabled');//预计到货日期
                    $('input[name="confirm_inventory"]').attr('disabled','disabled');//物资部库管员确认入库
                    $('input[name="delivery_type"]').attr('disabled','disabled');//发货类型
