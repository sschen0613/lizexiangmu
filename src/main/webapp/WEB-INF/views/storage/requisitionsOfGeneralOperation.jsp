	...
	<body>
	 		...
	 		<div class="layui-form-item">
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input name="curency_date2" type="text" class="layui-input input-revise date-revise" id="date1" placeholder="请选择开始日期">
					</div>
				</div>
				<div class="layui-inline">
				...
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input type="text" name="currency_string7" placeholder="客户名称 :" autocomplete="off" class="layui-input input-revise">
					</div>
				</div>
			  	<div class="layui-inline">
		...
		<script type="text/html" id="toolbarDemo">
			<div class="layui-btn-container">
				<button class="layui-btn layui-btn-xs" lay-event="add">运营、BO配件请购单</button>
			</div>
		</script>
...
// 				});
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Currency/selectApplicantCurrency.action?currency_type=10' //数据接口
					,page: true //开启分页
					,toolbar: '#toolbarDemo'
			    	,title: '普通运营请购申请'
...
// 						,{field: 'staff_name', title: '申请人'}
// 						,{field: 'department_name', title: '申请部门'}
						,{field: 'currency_date', title: '申请日期', sort: true,templet:'<div>{{Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
...
//						,{field: 'currency_money2', title: '实际已收款', minWidth:120}
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看明细
						...
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
							content: 'storage/requisitionsOfGeneralOperationFormRevise.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type //iframe的url
						});
				    	//同步更新缓存对应的值
				   		obj.update({
					    	// username: '123'
					...
					switch(obj.event){
						case 'add':
							layer.open({
								...
								maxmin: true,
								area: ['80%', '80%'],
								content: 'storage/requisitionsOfGeneralOperationForm.action' //iframe的url
							}); 
				    	break;
				    	case 'delete':
				    		layer.msg('删除');
