	...
	<body>
		...
		<script type="text/html" id="toolbarDemo">
			<div class="layui-btn-container">
				<button class="layui-btn layui-btn-xs" lay-event="add">内部出库申请表</button>
			</div>
		</script>
		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectApplicantCurrency1.action?currency_type=14';
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            // 3. 执行导出函数，系统会弹出弹框
                            LAY_EXCEL.exportExcel(data, '丽泽内部出库申请.xlsx', 'xlsx');
                        }
                    });
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Currency/selectApplicantCurrency.action?currency_type=14' //数据接口
					,page: true //开启分页
					,toolbar: '#toolbarDemo'
			    	,title: '丽泽内部出库申请'
// 			    	,totalRow: true //开启合计行
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看明细
						...
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
							content: 'storage/outlibraryOfAllocationFormRevise.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type //iframe的url
						});
				    	//同步更新缓存对应的值
				   		obj.update({
					    	// username: '123'
					...
					switch(obj.event){
						case 'add':
							layer.open({
								...
								maxmin: true,
								area: ['80%', '80%'],
								content: 'storage/outlibraryOfAllocationForm.action' //iframe的url
							}); 
				    	break;
				    	case 'delete':
				    		layer.msg('删除');
