...
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_date3', title: '期望到货日期', sort: true, minWidth:100, templet:'<div>{{ Format(d.currency_date3,"yyyy-MM-dd")}}</div>'}
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看
						layer.open({
							...
							content:'storage/requisitionsOfStockupSuppliesApprovalDetail1.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type
						});
						...
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=68"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '40%'],
							content: 'Currency/approvalOpinion.action?approval_id=68&currency_id='+data.currency_id
						});
					}
				});
