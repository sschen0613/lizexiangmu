				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看
						layer.open({
							...
							content:'storage/requisitionsOfStockupSuppliesApprovalDetail.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type
						});
						...
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=8"//iframe的url
						}); 
