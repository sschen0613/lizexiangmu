	...
	<body>
	 	<form class="layui-form form-top" action="">
	 		<div class="layui-form-item">
				...
				<div class="layui-inline">
					<label class="layui-form-label label-revise">编号:</label>
					<div class="layui-input-block">
						<input type="text" name="currency_number" placeholder="编号 :" autocomplete="off" class="layui-input input-revise">
					</div>
				</div>
			  	<div class="layui-inline">
		...
		<script type="text/html" id="toolbarDemo">
			<div class="layui-btn-container">
				<button class="layui-btn layui-btn-xs" lay-event="add">技术保障部配件采购单</button>
			</div>
		</script>
		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
				...
				//查询所有商品
// 				$.ajax({
// 					url : "storage/requisitionsOfStockupProductName.action",
// 					type : "post",
// 					data : {},
// 					dataType : "JSON",
...
// 							html += '<option value="">'+item.product_name+'</option>';
// 						});
// 						$('#inventory').html(html);
// 						form.render('select');
// 					}
// 				});
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Currency/selectApplicantCurrency.action?currency_type=73' //数据接口
					,page: true //开启分页
					,toolbar: '#toolbarDemo'
			    	,title: '技术保障部配件采购申请'
// 			    	,totalRow: true //开启合计行
...
// 						,{field: 'department_name', title: '申请部门', minWidth:100}
						...
						,{field: 'currency_date3', title: '期望到货日期', sort: true, minWidth:120,templet:'<div>{{Format(d.currency_date3,"yyyy-MM-dd")}}</div>'}
                        ,{field: 'currency_string2', title: '申请事由', minWidth:80}
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看明细
						...
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
							content: 'storage/requisitionsOfStockupFormRevise.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type //iframe的url
						});
				    	//同步更新缓存对应的值
				   		obj.update({
					    	// username: '123'
					...
					switch(obj.event){
						case 'add':
							layer.open({
								...
								maxmin: true,
								area: ['80%', '80%'],
								content: 'storage/technicalComponentsForm.action' //iframe的url
							}); 
				    	break;
				    	case 'delete':
				    		layer.msg('删除');
