	...
	<body>
		...
		<script type="text/html" id="toolbarDemo">
			<div class="layui-btn-container">
				<button class="layui-btn layui-btn-xs" lay-event="add">设备、配件销售请购单</button>
			</div>
		</script>
...
// 				});
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Currency/selectApplicantCurrency.action?currency_type=9' //数据接口
					,page: true //开启分页
					,toolbar: '#toolbarDemo'
			    	,title: '销售请购申请'
// 			    	,totalRow: true //开启合计行
...
// 						,{field: 'department_name', title: '申请部门', minWidth:100}
						...
						,{field: 'currency_date2', title: '期望到货日期', minWidth:180, sort: true, templet:'<div>{{ Format0(d.currency_date2,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_money', title: '合同总额', minWidth:100}
						,{field: 'currency_money2', title: '实际已收金额', minWidth:120}
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看明细
						...
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
							content: 'storage/requisitionsOfSaleFormRevise.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type //iframe的url
						});
				    	//同步更新缓存对应的值
				   		obj.update({
					    	// username: '123'
					...
					switch(obj.event){
						case 'add':
							layer.open({
								...
								maxmin: true,
								area: ['80%', '80%'],
								content: 'storage/requisitionsOfSaleForm.action' //iframe的url
							}); 
				    	break;
				    	case 'delete':
				    		layer.msg('删除');
