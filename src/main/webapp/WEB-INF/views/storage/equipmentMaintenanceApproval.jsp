		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectCurrencyApprover2.action?currency_type=16&currency_string='+staffid;
                    if (date1 != null && date1 != ''){
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            data.unshift({
                                ...
                                currency_string7: '故障描述',currency_string8: '技术要求',currency_string9: '其它说明',currency_string10: '维修建议厂家',
                                details_string2:"设备厂家", details_string5:"设施名称", details_string6:"规格型号",details_string7:"单位",
                                details_money:"数量",details_string11:"备注"
                            });
                            // 3. 执行导出函数，系统会弹出弹框
                            LAY_EXCEL.exportExcel(data, '信泽设备维修.xlsx', 'xlsx');
                        }
                    });
...
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_string7', title: '故障描述', minWidth:100}
						,{field: 'currency_string8', title: '技术要求', minWidth:100}
						,{field: 'currency_string9', title: '其它说明', minWidth:100}
						,{field: 'currency_string10', title: '维修建议厂家', minWidth:100}
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'print'){ //打印
                        layer.open({
                            ...
                            maxmin: true,
                            area: ['80%', '80%'],
                            content: 'storage/equipmentMaintenanceSuppliesApprovalPrint.action?currency_id='+data.currency_id+'&currency_type='+currency_type //iframe的url currency_id通用审批流主键
                        });
						...
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=16"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '50%'],
							content: 'Currency/approvalOpinion.action?approval_id=16&currency_id='+data.currency_id
						});
					}
				});
