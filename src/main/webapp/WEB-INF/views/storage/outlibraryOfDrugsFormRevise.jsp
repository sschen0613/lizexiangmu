...
<html>
	...
	<body>
		...
		<table class="layui-table table-open">
			<thead>
				<tr>
					<th colspan=8 class="table-h">药品出库申请表</th>
				</tr>
				<tr>
			...
			<tbody>
				...
				<tr>
					<td>所属公司</td>
					<td colspan=7><select name="subordinate_companies" id="subordinate_companies" lay-filter="subordinate_companies" lay-search></select></td>
				</tr>
				<tr>
					<td>仓库</td>
			...
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
				...
				//监听提交按钮
              	form.on('submit(submitForm)', function(data){
					...
					var subordinate_companies = $('#subordinate_companies').find('option:selected').text();
					...
					$.each($('.details'),function(index,item){
				 	...
				 	$.ajax({
				 		...
				 		,data : {
                            ...
                            'currency_string2':subordinate_companies
						}
				 		,dataType : "JSON"
				 		,beforeSend: function(){
					 		layer.msg('正在提交申请', {
                ...
                $.ajax({
                    ...
                    success:function(res){
                        ...
                        $('#subordinate_companies').html(html);
                        form.render('select');
                    }
                });
        		// 一:获取仓库
				...
				$.ajax({
                    ...
                    success:function(res){
            			...
            			var companyName = res.data.currency_string2;//公司名称
                        //初始化所属公司
                        $.ajax({
                            url:'Company/getCompany.action',
                            type:'post',
                            ...
                            success:function(res){
                                var html = '<option value="">请选择公司</option>';
                                $.each(res.data,function(index,item){
                                    html += '<option value="'+item.company_id+'" ';
                                    if(item.company_name == companyName){html += 'selected';}
                                    html += '>'+item.company_name+'</option>';
                                });
                                $('#subordinate_companies').html(html);
                                form.render('select');
                            }
