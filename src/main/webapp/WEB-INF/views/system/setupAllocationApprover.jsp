...
<form class="layui-form form-approval-persion">
	<div class="layui-form-item">
		...
		<div class="layui-inline ">
			<div class="layui-input-inline">
				<label class="layui-form-label">条件审批流程 :</label>
...
<table class="layui-hide" id="test" lay-data="{url:'/test/table/demo1.json', id:'demo'}" lay-filter="demo" lay-size="sm"></table>
...
layui.use(['laydate', 'layedit','laypage', 'layer', 'table',  'element','form'], function(){
	...
	//监听行双击事件
	  ...
	  //监听添加提交按钮
	  form.on('submit(search4)', function(data3){
		    layer.open({
		    	type: 2,
		    	title :'审批人',
		    	shade: 0.8,
        		...
        		move: false,
		    	area: ['90%', '90%'],
		    	content: '../Approval/setupConditionAllocationApprover.action?company_id='+company_id+'&approval_id='+approval_id, 
		        end: function () {
		        	table.reload("tab" ,{
		        	});
		          }
				 ...
				 layer.msg("请先进行选择");
				  ...
				  //添加审批角色
		 	        $.ajax({
			        	...
			        	type : "post",
			        	data : {"approver_name": rolename, "approver_id":role_id, "company_id":company_id, "role_type":role_type, "approval_id":approval_id},
			        	dataType : "JSON",
			        	success : function (result){
			        		layer.msg(result.msg);
							//重载表格
			 ...
			 layer.msg("请先进行选择");
			  ...
			  //添加审批人
	 	        $.ajax({
		        	url : "../Approval/addFacilitiesApproval.action",
		        	type : "post",
		        	data : {"approver_name": approver_name, "approver_id":staff_id, "company_id":company_id, "role_type":role_type, "approval_id":approval_id},
		        	dataType : "JSON",
		        	success : function (result){
		        		layer.msg(result.msg);
						//重载表格
		 ...
		 if(data2.department_id == ""){
			...
			//添加审批主管
	 	        $.ajax({
		        	url : "../Approval/addFacilitiesApproval.action",
		        	type : "post",
		        	data : {"approver_name": name, "approver_id": department_id, "role_type":role_type, "company_id":company_id, "approval_id":approval_id},
		        	dataType : "JSON",
		        	success : function (result){
		        		layer.msg(result.msg);
						//重载表格
	  ...
	  table.render({
	    elem: '#test'
	    //,height: 332
	    ,url: '../Approval/selectConditionApproval.action?approval_id='+approval_id+'&company_id='+company_id //数据接口
	    ,page: false //开启分页
	    ,id:'tab'
	    ,cols: [[ //表头
	      {field: 'approver_name', title: '角色名称'}
	      ,{field: 'role_priority', title: '审批顺序', edit: 'text'}
	      ,{field: 'role_state', title: '审批方式', sort: true, templet:'<div>{{ state(d.role_state)}}</div>'}
