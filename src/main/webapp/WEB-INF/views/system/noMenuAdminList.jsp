...
<script type="text/html" id="barDemo">
  <a class="layui-btn layui-btn-xs" lay-event="del">添加</a>
</script>
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
	<button class="layui-btn layui-btn-primary layui-btn-xs add" lay-event="add">批量添加</button>
  </div>
</script>
...
//监听搜索提交按钮
  ...
  table.render({
    elem: '#test'
    //,height: 332
    ,url: '../PowerMenu/selectNoMenu.action?menu_Priority='+position_id //数据接口
    ,toolbar:'#toolbarDemo'
    ,id:'tab'
    ,cols: [[ //表头
  ...
  table.on('tool(demo)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
    ...
    if(layEvent === 'detail'){
      ...
      layer.confirm('确认添加',{offset:'100'}, function(index){
        ...
        $.ajax({
        	url : "../ProwMenuallot/addMenuallot.action",
        	type : "post",
        	data : {"position_Id" : position_id, "menu_Id": data.menu_id},
        	dataType : "JSON",
  ...
  table.on('toolbar(demo)', function(obj){
    ...
    switch(obj.event){
      case 'add':
    	...
    	var position_id = ${param.position_Id};
             $.ajax({
            	url : "../ProwMenuallot/addMenuallots.action",
            	type : "post",
            	data : {"menu_ids" : menu_ids, "position_id": position_id},
            	traditional : true,
            	dataType : "JSON",
            	success : function (result){
            		layer.msg(result.msg);
