...
<div id="content" class="content">
	...
	<table border="1" class="layui-table message-table">
		...
		<%--<tr>
			<td>样品状态</td>
			<td colspan=4>
				<input type="text" name="state" id="state" lay-verify="required" style="border:none;">
			</td>
		</tr>--%>
		<tr>
			<td>检测项目类别</td>
	...
	<br/>
	<div class="subtitle"><h2>二、检测结果明细</h2></div>
	<table border="1" class="layui-table testing-table">
		<colgroup>
			<col width="200">
			<col width="100">
			<col width="100">
...
<script>
    layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
		...
		var currency_string17 = '${param.currency_string17}';
		// 表格一 :基本信息
		$.ajax({
			...
			success:function(res){
				...
				//$('#adress').text(res.data.currency_string5); //委托单位地址
				//$('#client').text(res.data.currency_string3); //委托人
				//$('#phone').text(res.data.currency_string4); //联系电话
				$('#source').text(res.data.currency_int2 == 1 ? "现场采样" : "送样"); //样品来源
				$('#purpose').text(res.data.currency_string8); //检测目的
				if(res.data.currency_date3 == res.data.currency_date2){
                    $('#sampling_date').text(Format(res.data.currency_date2,"yyyy-MM-dd"));
				}else {
                    $('#sampling_date').text(Format(res.data.currency_date3,"yyyy-MM-dd")+"-"+Format(res.data.currency_date2,"yyyy-MM-dd"));
				}
                if(res.data.currency_date5 == res.data.currency_date4){
                    $('#analysis_date').text(Format(res.data.currency_date4,"yyyy-MM-dd"));
                }else {
                    $('#analysis_date').text(Format(res.data.currency_date5,"yyyy-MM-dd")+"-"+Format(res.data.currency_date4,"yyyy-MM-dd"));
                }
				//$('#state').text(); //样品状态
				var html = '';
				$.each(res.data2,function(index,item){
					html += '<tr>'
						...
						+		'<td colspan=3>'+item.processName+'</td>'
						+		'<td>'+item.details_int3+'次/天,'+item.details_int4+'天</td>'
						+	'</tr>';
				});
		...
		$.ajax({
			...
			data:{
				...
				"currency_int2":currency_int2,
				"currency_string17":currency_string17
			},
			dataType:'JSON',
			success:function(res){
                var currentClass = '', currentRow = 1;
                $.each(res.data,function (index,item) {
                    ...
                    if(currentClass != item.details_string){
                        ...
                        $('#content>table.testing-table>tbody>tr').eq(-currentRow).find('td').eq(0).attr('rowspan',(++currentRow));
                    }
                    html += '<td>'+item.details_string2+'</td>'
                        +	'<td>'+item.details_string3+'</td>'
                        +	'<td>'+item.details_string9+'</td>'
                        +	'</tr>';
                    $('#content>table.testing-table>tbody').append(html);
                });
			}
		});
		//表格四：检测期间参数统计表
