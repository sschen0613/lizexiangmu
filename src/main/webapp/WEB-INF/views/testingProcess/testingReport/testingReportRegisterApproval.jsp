...
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: '', title: '任务下达时间', sort: true, minWidth:120, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
						,{field: '', title: '检测类型', minWidth:100}
						,{field: '', title: '要求完成时间', sort: true, minWidth:120, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
						,{field: '', title: '执行标准', minWidth:100}
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '40%'],
							content: 'Currency/approvalOpinion.action?approval_id=8&currency_id='+data.currency_id
						});
					}
				});
