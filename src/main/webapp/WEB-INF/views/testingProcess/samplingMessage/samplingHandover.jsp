	 		...
	 		<div class="layui-form-item">
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input type="text" name="currency_string17" required  lay-verify="" placeholder="请输入样品名称" autocomplete="off" class="layui-input input-revise">
					</div>
				</div>
				<div class="layui-inline">
				...
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input name="currency_date2" type="text" class="layui-input input-revise date-revise" id="date1" placeholder="请选择采样日期">
					</div>
				</div>
				<div class="layui-inline">
					<label class="layui-form-label label-revise">结束日期 :</label>
					<div class="layui-input-block">
						<input name="currency_date3" type="text" class="layui-input input-revise date-revise" id="date2" placeholder="请选择采样日期">
					</div>
				</div>
			  	<div class="layui-inline">
...
<!-- 				<button class="layui-btn layui-btn-xs" lay-event="add">样品编码登记表</button> -->
<!-- 			</div> -->
<!-- 		</script> -->
...
<!-- 		<a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="approval">审批详情</a> -->
		<script type="text/html" id="barDemo">
			<a class="layui-btn layui-btn-xs" lay-event="next">任务交接表</a>
		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Xinze/selectCaiyangOrShangJiApplicantCurrency.action?currency_type=44&currency_int=1' //数据接口
					,page: true //开启分页
					,toolbar: true
			    	,title: '采样交接'
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_date', title: '登记时间', minWidth:150, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
						,{field: 'staff_name', title: '登记人', minWidth:150}
						...
						,{field: 'currency_date2', title: '任务下达时间', minWidth:120, sort: true, templet:'<div>{{ Format(d.currency_date2,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string8', title: '检测类型', minWidth:100}
						,{field: 'currency_date3', title: '要求完成时间', minWidth:120, sort: true, templet:'<div>{{ Format(d.currency_date3,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string9', title: '执行标准', minWidth:200}
						
						,{fixed: 'right', title:'操作', toolbar: '#barDemo', minWidth:320}
					]]
				});
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'detail'){ //查看
                        layer.open({
                            ...
                            maxmin: true,
                            area: ['80%', '80%'],
                            content: 'Currency/currencyDetails.action?currency_id='+data.currency_id+'&currency_type='+45 //iframe的url currency_id通用审批流主键
                        });
						...
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
							content: 'testingProcess/samplingMessage/samplingHandoverForm.action?currency_id='+data.currency_id //iframe的url
						}); 
					} else if(layEvent === 'form1'){ //样品编码表
						layer.open({
							type: 2,
							// skin:'layui-layer-molv', //layui-layer-lan
							title: '样品编码表',
					...
					switch(obj.event){
						case 'add':
							layer.open({
								...
								maxmin: true,
								area: ['80%', '80%'],
								content: 'testingProcess/samplingMessage/samplingHandoverForm.action' //iframe的url
							}); 
				    	break;
				    	case 'delete':
				    		layer.msg('删除');
