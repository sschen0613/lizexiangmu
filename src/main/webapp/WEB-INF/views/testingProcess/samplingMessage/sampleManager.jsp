...
<script>
    ...
    layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    data.unshift({
                        ...
                        currency_string2:'项目名称',currency_string3: '联系人',currency_string4: '联系电话',currency_string5: '委托单位地址',
                        currency_string10:'委托单位名称',currency_string7: '报表编码',currency_date2: '采样完成时间',currency_string8: '检测类型',
                        currency_date3:"协议完成时间",currency_string9:"执行标准"
                    });
                    // 3. 执行导出函数，系统会弹出弹框
                    LAY_EXCEL.exportExcel(data, '采样任务通知.xlsx', 'xlsx');
                }
            });
        ...
        var tableInner = table.render({
            elem: '#tab'
            ,url: 'Xinze/selectApplicantCurrency.action?currency_type=43&currency_int=1' //数据接口
            ,page: true //开启分页
            ,toolbar: true
            ,title: '检测任务登记'
// 			    	,totalRow: true //开启合计行
            ,cols: [[ //表头
                ...
                ,{field: 'currency_date3', title: '协议完成时间', minWidth:150, sort: true, templet:'<div>{{ Format0(d.currency_date3,"yyyy-MM-dd HH:ss:mm")}}</div>'}
            ...
            if(layEvent === 'detail'){ //查看明细
                ...
                layer.open({
                    ...
                    content: 'testingProcess/samplingMessage/sampleManagerForm.action?currency_id='+data.currency_id
							...
							+'&currency_string2='+data.currency_string2+'&currency_string8='+data.currency_string8
