	...
	<body>
	 		...
	 		<div class="layui-form-item">
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input type="text" name="currency_string2" required  lay-verify="" placeholder="请输入联系人" autocomplete="off" class="layui-input input-revise">
					</div>
				</div>
				<div class="layui-inline">
		...
		<script type="text/html" id="toolbarDemo">
			<div class="layui-btn-container">
				<button class="layui-btn layui-btn-xs" lay-event="add">已填写通知单列表</button>
			</div>
		</script>
<!-- 		<a class="layui-btn layui-btn-xs" lay-event="edit">编辑</a> -->
		<script type="text/html" id="barDemo">
			<a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="add">采样任务通知单</a>
		</script>
		<script>
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Xinze/selectApplicantCurrency.action?currency_type=51&currency_int=5' //数据接口
					,page: true //开启分页
					,toolbar: '#toolbarDemo'
			    	,title: '采样任务通知'
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_number', title: '任务编号', minWidth:200, sort:true}
						,{field: 'staff_name', title: '下发人', minWidth: 100}
						,{field: 'currency_date', title: '下发日期', minWidth: 120, sort: true, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
                        ,{field: 'currency_string3', title: '委托单位名称', minWidth:120}
						,{field: 'currency_string4', title: '合同编号', minWidth: 150}
						,{field: 'currency_string2', title: '样品来源', minWidth: 100}
						,{field: 'currency_string5', title: '任务内容', minWidth: 200}
						...
						,{fixed: 'right', title:'操作', toolbar: '#barDemo', minWidth:130}
					]]
				});
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看明细
						...
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
							content: 'testingProcess/samplingMessage/samplingTaskNoticeForm.action?currency_id='+data.currency_id //iframe的url
						});
					} else if(layEvent === 'print'){ //采样任务登记表
						layer.open({
							type: 2,
							// skin:'layui-layer-molv', //layui-layer-lan
							title: '采样任务通知单打印',
							...
							maxmin: true,
							area: ['80%', '80%'],
							content: 'testingProcess/samplingMessage/samplingTaskNoticePrint.action?currency_id='+data.currency_id //iframe的url
						});
					}
				});
				//监听头工具栏事件
					...
					switch(obj.event){
						case 'add':
							layer.open({
								...
								maxmin: true,
								area: ['80%', '80%'],
								content: 'testingProcess/samplingMessage/samplingTaskNoticeList.action' //iframe的url
							}); 
				    	break;
				    	case 'delete':
				    		layer.msg('删除');
