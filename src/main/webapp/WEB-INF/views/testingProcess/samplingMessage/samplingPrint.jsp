	 		...
	 		<div class="layui-form-item">
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input type="text" name="currency_string2" required  lay-verify="" placeholder="请输入任务下达人" autocomplete="off" class="layui-input input-revise">
					</div>
				</div>
				<div class="layui-inline">
...
<!-- 		</script> -->
		<script type="text/html" id="barDemo">
			<a class="layui-btn layui-btn-xs" lay-event="print">通知单打印</a>
		</script>
		<script>
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Xinze/selectApplicantCurrency.action?currency_type=43' //数据接口
					,page: true //开启分页
					,toolbar: true
			    	,title: '采样任务登记'
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_date2', title: '任务下达时间', minWidth:120, sort: true, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string8', title: '检测类型', minWidth:100}
						,{field: 'currency_date3', title: '要求完成时间', minWidth:120, sort: true, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'print'){ //查看明细
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
							content: 'testingProcess/samplingMessage/samplingTaskNoticePrint.action?currency_id='+data.currency_id //iframe的url currency_id通用审批流主键
						});
