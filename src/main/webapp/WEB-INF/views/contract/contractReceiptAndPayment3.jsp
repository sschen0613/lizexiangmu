...
<form class="layui-form form-top" action="">
	...
	<div class="layui-form-item">
		...
		<div class="layui-inline">
			...
			<div class="layui-input-block">
				<input type="text" name="currency_string3" placeholder="签订日期起" id="date3" class="layui-input input-revise date-revise">
			</div>
		</div>
		...
		<div class="layui-inline">
			...
			<div class="layui-input-block">
				<input type="text" name="currency_string7" placeholder="签订日期止" id="date4" class="layui-input input-revise date-revise">
			</div>
		</div>

		<div class="layui-inline">
			<label class="layui-form-label label-revise">合同开始日期 :</label>
			<div class="layui-input-block">
				<input name="currency_string4" type="text" class="layui-input input-revise date-revise" id="date1" placeholder="合同开始日期">
			</div>
		</div>

		<div class="layui-inline">
			<label class="layui-form-label label-revise">合同结束日期 :</label>
			<div class="layui-input-block">
				<input name="currency_string5" type="text" class="layui-input input-revise date-revise" id="date2" placeholder="合同结束日期">
			</div>
		</div>
...
<script>
    ...
    layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
        ...
        form.on('submit(out)', function(data){
            ...
            var currency_string4 = $('input[name="currency_string4"]').val();//合同开始时间
            var currency_string5 = $('input[name="currency_string5"]').val();//合同结束时间
            var currency_string8 = $('input[name="currency_string8"]').val();//来款时间起
            var currency_string9 = $('input[name="currency_string9"]').val();//来款时间止

            var url = 'Currency/selectContractStatement.action?currency_string6=03';
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    data.unshift({
                        ...
                        ,strName: '标的名称',strCorrItemID: '对应项目编码',dblQuantity:"数量",dblPrice:"含税原币单价",dblSum:"含税原币金额"
                        ,dblTotalCurrency:"合同总金额（元）", receiveAmount:"已付金额（元）",spareMaoney:"欠款金额（元）"
                        ,cDefine12:"合同期间金额（1）",cDefine13:"合同期间金额（2）"
                    ...
                    // 3. 执行导出函数，系统会弹出弹框
                    LAY_EXCEL.exportExcel(data, '技术服务类合同报表.xlsx', 'xlsx');
                }
            });
        ...
        var tableInner = table.render({
            elem: '#tab'
            ,url: 'Currency/selectContractStatement.action?currency_string6=03'//数据接口
            ,page: true //开启分页
            ,toolbar: '#toolbarDemo'
            ,title: '合同报表'
// 			,totalRow: true //开启合计行
            ,cols: [[ //表头
                ...
                ,{field: 'strCorrItemID', title: '对应项目编码', minWidth:200, sort:true}
                ...
                ,{field: 'cDefine12', title: '合同期间金额（1）', minWidth:130, sort:true}
                ,{field: 'cDefine13', title: '合同期间金额（2）', minWidth:130, sort:true}
                /*,{field: '', title: '合同期间', minWidth:200, sort:true, templet:'<div>{{ subDate(d.strContractStartDate)}} - {{ subDate(d.strContractEndDate)}}</div>'}*/
                ,{field: 'strContractStartDate', title: '合同起', sort: true, minWidth:100, templet:'<div>{{ subDate(d.strContractStartDate)}}</div>'}
                ,{field: 'strContractEndDate', title: '合同止', sort: true, minWidth:100, templet:'<div>{{ subDate(d.strContractEndDate)}}</div>'}
