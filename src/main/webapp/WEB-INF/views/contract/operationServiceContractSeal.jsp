	...
	<body>
		...
		<script type="text/html" id="toolbarDemo">
			<div class="layui-btn-container">
				<button class="layui-btn layui-btn-xs" lay-event="add">丽泽运营服务合同盖章申请单</button>
			</div>
		</script>
...
// 				});
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Currency/selectApplicantCurrency.action?currency_type=35' //数据接口
					,page: true //开启分页
					,toolbar: '#toolbarDemo'
			    	,title: '丽泽运营服务合同盖章申请'
// 			    	,totalRow: true //开启合计行
...
//						,{field: 'contract_versons', title: '合同版本', minWidth:100, templet:'<div>{{d.contract_versons == "1" ? "我方" : "对方"}}</div>'}
...
//						,{field: 'currency_int7', title: '金额', minWidth:80}
						,{field: 'currency_date2', title: '合同签订日期', minWidth:120, sort: true, templet:'<div>{{ Format(d.currency_date2,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string', title: '配件特殊约定', minWidth:200}
						,{field: 'currency_string15', title: '检测比对约定', minWidth:200}
					...
					switch(obj.event){
						case 'add':
							layer.open({
								type: 2,
								// skin:'layui-layer-molv', //layui-layer-lan
								title: '丽泽运营服务合同盖章申请单填写',
								//shadeClose: true,
								shade: 0.8,
								maxmin: true,
								area: ['80%', '80%'],
								content: 'contract/operationServiceContractSealForm.action' //iframe的url
							}); 
				    	break;
				    	case 'delete':
				    		layer.msg('删除');
