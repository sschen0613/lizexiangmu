...
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_date2', title: '故障发生日期', minWidth:120, sort: true, minWidth:100, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string11', title: '故障描述', minWidth:100}
						,{field: 'currency_string7', title: '申请原因', minWidth:100}
						,{field: 'currency_string13', title: '维修建议厂家', minWidth:120}
					...
					if(layEvent === 'del'){ //删除
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=46"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '50%'],
							content: 'Currency/approvalOpinion.action?approval_id=46&currency_id='+data.currency_id
						});
					}
				});
