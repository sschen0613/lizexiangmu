	...
	<body>
		...
		<script type="text/javascript">
            layui.use(['form','element','layer','laydate','flow' ],function(){
                ...
                //数据初始化
    			$.ajax({
    				url:'Currency/selectApplicantCurrency.action',
    				type:'post',
    				dataType:'JSON',
    				beforeSend: function(){
    					layer.load(1);
...
//    				    	myApply(item,index,state);
                            ...
                            html += '<li>'
                                ...
                                +		'<i>申请类别 : '+item.approval_name+'</i>'
                                +		'<a href='+item.approver_url+'?currency_id='+item.currency_id
...
//     			});
            ...
            function Url(type) {
                var result = "";
                $.ajax({
                    url : "Currency/findApproverUrlByTyep.action",
                    type : "post",
                    data : {"currency_type" : type},
                    dataType : "JSON",
                    async:false,
