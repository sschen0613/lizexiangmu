...
<div class="container-approval">
    <h2 class="title-page">
        <svg class="alSvgIcon" aria-hidden="true">
            <use xlink:href="#icon-l-debugging1"></use>
        </svg>
        检测科绩效申请详情
    </h2>
    <div class="content">
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                html += '<div class="content-list style">'
                    +		'<form>'
                    +			'<div>'
                    +				'<label class="">申请单号 :</label>'
                    +				'<input type="text" id="buy_number" name="buy_number" value="'+currency_number+'" class="layui-input" readonly>'
                    +			'</div>'
                    +			'<div>'
                    +				'<label class="">申请人 :</label>'
                    +				'<input type="text" name="currency_applicant" id="currency_applicant" value="'+staff_name+'" class="layui-input" readonly>'
                    +			'</div>'
                    +			'<div>'
                    +				'<label class="">申请部门 :</label>'
                    +				'<input type="text" name="department_Id" id="department_Id" value="'+department_name+'" class="layui-input" readonly>'
                    +			'</div>'
                    +			'<div>'
                    +				'<label class="">申请日期 :</label>'
                    +				'<input type="text" name="currency_date" id="currency_date" value="'+currency_date+'" class="layui-input" readonly>'
                    +			'</div>'
                    +			'<div>'
                    ...
                    success:function(res){
                        ...
                        $.each(res.data,function(index,item){
                            html0+=	'<div class="approval-detail">'
                                ...
                                +		'<div class="details">'
                                +			'<div>'
                                +				'<label class="">检测时间 :</label> '
                                +				'<input type="text" name="test_date" value="'+Format(item.details_date2,'yyyy-MM-dd')+'" class="layui-input" readonly>'
                                +			'</div>'
                                +			'<div>'
                                +				'<label class="">企业名称/样品名称/编号 :</label>'
                                +				'<input type="text" name="name" value="'+item.details_string2+'" class="layui-input" readonly>'
                                +			'</div>'
                                +			'<div>'
                                +				'<label class="">分析内容 :</label>'
                                +				'<input type="text" name="content" value="'+item.details_string3+'" class="layui-input" readonly>'
                                +			'</div>'
                                +			'<div>'
                                ...
                                +			'</div>'
                                +			'<div>'
                                +				'<label class="">绩效单价 :</label>'
                                +				'<input type="text" name="price" value="'+item.details_money2+'" class="layui-input" readonly>'
                                +			'</div>'
                                +			'<div>'
                                +				'<label class="">绩效金额 :</label>'
                                +				'<input type="text" name="sum" value="'+item.details_money3+'" class="layui-input" readonly>'
                                +			'</div>'
                                +			'<div>'
