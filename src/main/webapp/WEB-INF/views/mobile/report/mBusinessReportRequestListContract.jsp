    ...
    <div class="approval-list-content">
        <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
            <div class="layui-tab-content" style="height:100%;">
                <div class="layui-tab-item adopt layui-show">
                    <ul id="adopt_demo">
                        <!-- 已通过 -->
                    </ul>
                </div>
            ...
            success:function(res){
                ...
                $.each(res.data,function(index,item){
                    ...
                    html += '<li>'
                        ...
                        +		'<a href="report/mBusinessReportRequestAddContract.action?currency_id='+item.currency_id
                        +'&current_approvalCount='+item.current_approvalCount
                        +'&approver_count='+item.approver_count
                        +'&approvalOpinion_type=69&approval_id=69'
                        ...
                        +'&currency_string2='+item.currency_string2+'&currency_date2='+Format(item.currency_date2,"yyyy-MM-dd")+'&currency_date3='+Format(item.currency_date3,"yyyy-MM-dd")
                        +'&currency_string2='+item.currency_string3+'&currency_string2='+item.currency_string4
                        +'&currency_string2='+item.currency_string5+'&currency_string2='+item.currency_string9
                        +'&currency_string7='+item.currency_string7+'&currency_string8='+item.currency_string8+'&currency_string10='+item.currency_string10
                    ...
                    //判断审批状态,保存页面信息
                    //审批通过
                    if(item.approver_count == item.current_approvalCount && item.approver_state == 0){
...
//     			});
        ...
        element.on('tab(docDemoTabBrief)', function(data){
           ...
           if(data.index == 1 && A_List.adopt[0].count > 0){
                $('#adopt_demo').html('');
                //审批通过流加载初始化
                flow.load({
                    elem: '#adopt_demo' //流加载容器
