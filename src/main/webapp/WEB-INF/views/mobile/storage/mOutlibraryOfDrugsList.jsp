    				...
    				success:function(res){
    					...
    					$.each(res.data,function(index,item){
    						...
    						html += '<li>'
    				    		 ...
    				    		 +		'<a href="storage/mOutlibraryOfDrugsDetails.action?currency_id='+item.currency_id
    				    				 +'&current_approvalCount='+item.current_approvalCount
    				    				 +'&approver_count='+item.approver_count
    				    				 +'&approvalOpinion_type=18&approval_id=18'
    				    				 ...
    				    				 +'&currency_string7='+item.currency_string7+'&currency_string2='+item.currency_string2+'&currency_string8='+item.currency_string8
    				    				 +'&approverState='+state
    				    				 +'&currency_type='+currency_type+'" id="a'+index+'" onclick="approvalDetails(this);return false;">'
    				    		 +			'查看详情'
...
//     			});
    			...
    			element.on('tab(docDemoTabBrief)', function(data){
    				...
    				if(data.index == 0 && A_List.waitting[0].count == 0){
    					$('.'+A_List.waitting[0].name+'>ul').html('');
    					$('.'+A_List.waitting[0].name).addClass('approval-none');	
    				}else if(data.index == 0 && A_List.waitting[0].count > 0){
    					$('#waitting_demo').html('');
    					//审批未处理流加载初始化
    					flow.load({
    	                	elem: '#waitting_demo' //流加载容器
