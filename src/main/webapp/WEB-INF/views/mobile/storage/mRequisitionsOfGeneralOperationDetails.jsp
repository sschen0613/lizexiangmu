	...
	<body>
		...
		<script type="text/javascript">
            layui.use(['form','element','layer','laydate', 'table' ],function(){
                ...
                var currency_int3 = '${param.currency_int3}';
                var currency_date2 = '${param.currency_date2}'; //期望到货日期
                var currency_date4 = '${param.currency_date4}'; //期望到货日期
                        ...
                        $.ajax({
                            ...
                            success:function(res){
                                ...
                                html += '<div class="content-list style">'
                                    ...
                                    +				'<input type="text" id="delivery_type" name="delivery_type" value="'+(currency_int2==1?'BO设备出库':'运营设备出库')+'" class="layui-input" readonly>'
                                    +			'</div>'*/
                                    +			'<div>'
                                    +				'<label class="">申请类型 :</label>'
                                    +				'<input type="text" id="applyType" name="applyType" value="'+(currency_int3==1?'普通请购':'维修入库')+'" class="layui-input" readonly>'
                                    +			'</div>'
                                    +			'<div>'
                                    +				'<label class="">货品流转编码 :</label>'
                                    +				'<input type="text" id="productNumber" name="productNumber" value="'+(currency_string9=='undefined'?'无':currency_string9)+'" class="layui-input" readonly>'
                                    +			'</div>'
                                    +			'<div>'
                                    +				'<label class="">运营合同起止时间 :</label>'
                                    +				'<input type="text" name="beginning_endingtime" id="date1" value="'+currency_date2+'" class="layui-input" readonly>-'
                                    +				'<input type="text" name="endning_endingtime" id="date1" value="'+currency_date4+'" class="layui-input" readonly>'
                                    +			'</div>'
                                    +			'<div>'
                                ...
                                html +=			'</div>'
                                    +			'<div class="button">'
                                    +				'<button type="button" class="layui-btn layui-btn-danger layui-btn-sm" id="revokeagree">拒绝</button>'
                                    +			'</div>'
                                    +		'</form>'
                                    +	'</div>';
                                $('.content').append(html);
                                    ...
                                    success:function(res){
                                        ...
                                        $.each(res.data,function(index,item){
                                            html0+=	'<div class="approval-detail">'
                                                ...
                                                +				'<input type="text" name="start_date" value="'+item.details_date2+'" class="layui-input" readonly>'
                                                +			'</div>'
                                                +			'<div>'
                                                +				'<label class="">设备结束日期 :</label>'
                                                +				'<input type="text" name="over_date" value="'+item.details_date3+'" class="layui-input" readonly>'
                                                +			'</div>'*/
                                                +			'<div>'
