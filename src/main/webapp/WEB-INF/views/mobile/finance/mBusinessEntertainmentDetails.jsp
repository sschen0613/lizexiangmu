	...
	<body>
		...
		<script type="text/javascript">
            layui.use(['form','element','layer','laydate', 'table' ],function(){
                ...
                var currency_int6 = '${param.currency_int6}'; //合同金额
                var currency_money6 = '${param.currency_money6}'; //已收金额
                var currency_date2 = '${param.currency_date2}'; //招待时间
    			...
    			$.ajax({
    				...
    				success:function(res){
						...
						html += '<div class="content-list style">'
							 ...
							 +				'<input type="text" id="customer_unit" name="customer_unit" value="'+currency_string4+'" class="layui-input" readonly>'
							 +			'</div>'
							 +			'<div>'
							 +				'<label class="">客户人数 :</label>'
							 +				'<input type="text" name="person_number" value="'+currency_int6+'" class="layui-input" readonly>'
							 +			'</div>'
							 +			'<div>'
							 +				'<label class="">陪同人 :</label>'
							 +				'<input type="text" name="accompanying_person" value="'+currency_string8+'" class="layui-input" readonly>'
							 +			'</div>'
                             +			'<div>'
                             +				'<label class="">招待时间 :</label>'
                             +				'<input type="text" name="serve_date" value="'+currency_date2+'" class="layui-input" readonly>'
                             +			'</div>'
							 +			'<div>'
							 +				'<label class="">招待事由 :</label>'
							 +				'<input name="serve_reason" value="'+currency_string7+'" class="layui-input" readonly>'
							 +			'</div>'
							 +			'<div>'
							 +				'<label class="">申请金额 :</label>'
							 +				'<input name="applied_amount" value="'+currency_money6+'" class="layui-input" readonly>'
							 +			'</div>'
							 +			'<div>'
							 +				'<label class="">申请金额大写 :</label>'
							 +				'<input name="applied_ChineseAmount" value="'+currency_string5+'" class="layui-input" readonly>'
							 +			'</div>'
