...
<script type="text/javascript">
    layui.use(['form','element','layer','laydate','flow' ],function(){
        ...
        $.ajax({
            url:'Currency/selectCurrencyApprover.action?currency_string='+staffid,
            type:'post',
            dataType:'JSON',
            beforeSend: function(){
                layer.load(1);
            ...
            success:function(res){
                ...
                $.each(res.data,function(index,item){
                    ...
                    //未通过
                    else if(item.approval_result == 2){state = 'reject';}
                    //审批通过
                    else if(item.approval_result == 1){state = 'adopt';}
                    //审批未处理
                    else if(item.approval_result == 0){state = 'waitting';}
//    				    	myApply(item,index,state);
                    ...
                    if(item.currency_type == 43){
                        html += '<i>业务名称 : '+item.currency_string2+'</i>';
					}

                        html +=	'<i>申请类别 : '+item.approval_name+'</i>'
                        +		'<a href='+item.approval_action+'?currency_id='+item.currency_id
                        +		'&dingStaffid='+staffid
                        +'&current_approvalCount='+item.current_approvalCount
                        +'&approver_count='+item.approver_count
                        +'&approvalOpinion_type='+item.currency_type+'&approval_id='+item.currency_type
                        ...
                        +'&currency_string='+item.currency_string+'&currency_string2='+item.currency_string2+'&currency_string3='+item.currency_string3
                        +'&currency_string4='+item.currency_string4+'&currency_string5='+item.currency_string5+'&currency_string6='+item.currency_string6
                        +'&currency_string7='+item.currency_string7+'&currency_string8='+item.currency_string8+'&currency_string9='+item.currency_string9
                        +'&currency_string10='+item.currency_string10+'&currency_string11='+item.currency_string11+'&currency_string12='+item.currency_string12
                        +'&currency_string13='+item.currency_string13+'&currency_string14='+item.currency_string14+'&currency_string15='+item.currency_string15
                        +'&currency_string16='+item.currency_string16+'&currency_string17='+item.currency_string17+'&currency_string18='+item.currency_string18
                        +'&currency_int='+item.currency_int+'&currency_int2='+item.currency_int2+'&currency_int3='+item.currency_int3
                        +'&currency_int4='+item.currency_int4+'&currency_int5='+item.currency_int5+'&currency_int6='+item.currency_int6
                        +'&currency_int7='+item.currency_int7
                        +'&currency_date2='+Format(item.currency_date2,"yyyy-MM-dd") +'&currency_date3='+Format(item.currency_date3,"yyyy-MM-dd")
                        +'&currency_date4='+Format(item.currency_date4,"yyyy-MM-dd") +'&currency_date5='+Format(item.currency_date5,"yyyy-MM-dd")
                        +'&currency_money='+item.currency_money+'&currency_money2='+item.currency_money2+'&currency_money3='+item.currency_money3
                        +'&currency_money4='+item.currency_money4+'&currency_money5='+item.currency_money5+'&currency_money6='+item.currency_money6
                    ...
                    //未通过
                    else if(item.approval_result == 2){
                        A_List.reject[A_List.reject.length] = html;
                        A_List.reject[0].count++;
                    }
                    //审批通过
                    else if(item.approval_result == 1){
...
//     			});
    ...
    function Action(type) {
        var result = "";
        $.ajax({
            url : "Currency/findApprovalActionByTyep.action",
            type : "post",
            data : {"currency_type" : type},
            dataType : "JSON",
            async:false,
