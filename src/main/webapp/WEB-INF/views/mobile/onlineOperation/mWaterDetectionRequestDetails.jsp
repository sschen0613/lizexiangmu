	...
	<body>
		...
		<script type="text/javascript">
            layui.use(['form','element','layer','laydate', 'table' ],function(){
                ...
                var currency_date2 = '${param.currency_date2}'; //申请事由
                var currency_date3 = '${param.currency_date3}'; //申请事由
                var currency_date4 = '${param.currency_date4}'; //申请事由
    			...
    			$.ajax({
    				...
    				success:function(res){
						...
						html += '<div class="content-list style">'
							 ...
							 +			'</div>'
                            +			'<div>'
                            +				'<label class="">申请区域 :</label>'
                            +				'<input type="text" name="area" value="'+currency_string2+'" readonly>'
                            +			'</div>'
                            +			'<div>'
                            +				'<label class="">企业名称 :</label>'
                            +				'<input type="text" name="company_name" value="'+currency_string7+'" readonly>'
                            +			'</div>'
                            +			'<div>'
                            +				'<label class="">采样日期 :</label>'
                            +				'<input type="text" name="currency_date4" id="date4" value="'+currency_date4+'" class="layui-input" readonly>'
                            +			'</div>'
                            +			'<div>'
                            +				'<label class="">计划检测日期 :</label>'
                            +				'<input type="text" name="currency_date2" id="date2" value="'+currency_date2+'" class="layui-input" readonly>'
                            +			'</div>'
                            +			'<div>'
                            +				'<label class="">计划完成日期 :</label>'
                            +				'<input type="text" name="currency_date3" id="date3" value="'+currency_date3+'" class="layui-input" readonly>'
                            +			'</div>'
							...
							success:function(res){
								...
								$.each(res.data,function(index,item){
									html0+=	'<div class="approval-detail">'
							 		 	 ...
							 		 	 +				'<input type="text" name="enterprise_name" value="'+item.details_string2+'" class="layui-input" readonly>'
							 		 	 +			'</div>'
							 		 	 +			'<div>'
							 		 	 +				'<label class="">检测数量 :</label>'
							 		 	 +				'<input type="text" name="equipment_name" value="'+item.details_money3+'" class="layui-input" readonly>'
							 		 	 +			'</div>'
							 		 	 +		'</div>'
							 		 	 +	'</div>';
								});
