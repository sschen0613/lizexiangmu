...
<html>
	...
	<body>
		...
		<table class="layui-table table-open">
			<thead>
				<tr>
					<th colspan=6 class="table-h">丽泽公司运营合同发票开具请购单</th>
				</tr>
				<tr>
					<th colspan=6 style="text-align:right">请购单号：<input type="text" id="buy_number" name="buy_number"> </th>
				</tr>
			</thead>
			<tbody>
				<tr>
				...
				<tr>
					<td>开票名称</td>
					<td colspan=5><input type="text" name="invoice_name" id="invoice_name" readonly></td>
				</tr>
				<tr>
					<td>税号（社会统一信用码）</td>
					<td colspan=5><input type="text" name="tax_number" id="tax_number" readonly></td>
				</tr>
				<tr>
					<td>地址</td>
					<td colspan=2><input type="text" id="site" name="site" readonly></td>
					<td>电话</td>
					<td colspan=2><input type="text" id="telephone" name="telephone" readonly></td>
				</tr>
				<tr>
					<td>开户行</td>
					<td colspan=2><input type="text" id="bank" name="bank" readonly></td>
					<td>账号</td>
					<td colspan=2><input type="text" id="account" name="account" readonly></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td colspan=2>
						<%--<div class="layui-inline time-ranges">
							<input type="text" name="start_time0" class="layui-input" id="start_time0" placeholder="选择开始时间" autocomplete="off" lay-verify="required">
							&nbsp;-&nbsp;
							<input type="text" name="end_time0" class="layui-input" id="end_time0" placeholder="选择结束时间" autocomplete="off" lay-verify="required">
						</div>--%>
							<div class="layui-inline time-ranges">
								<input type="text" name="start_time0" class="layui-input" id="start_time0" readonly>
								&nbsp;-&nbsp;
								<input type="text" name="end_time0" class="layui-input" id="end_time0" readonly>
							</div>
					</td>
					<td>本次开票起止日期</td>
					<td colspan=2>
				...
				<tr>
					<td>开票信息是否一致</td>
					<td colspan=5>
						<input type="radio" name="choose" value="1" title="是" checked>
						<input type="radio" name="choose" value="2" title="否">
					</td>
				</tr>
			</tbody>	
		 	<thead>
			...
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
				...
				/*laydate.render({
			        ...
			        ,done: function(value, date, endDate){
				    	...
				    	//开始日选好后，判断结束日期是否合法
				    	if($('#end_time0').val() != ''){
				    		$('#end_time0').val() > value ? 1 : layer.msg('请选择正确日期范围',{time: 1000},function(){$('#start_time0').val('');});
				    	}
				    }
				});
				laydate.render({
			        ...
			        ,done: function (value, date, endDate) {
				    	...
				    	//结束日选好后，判断开始日期是否合法
				    	if($('#start_time0').val() != ''){
				    		$('#start_time0').val() < value ? 1 : layer.msg('请选择正确日期范围',{time: 1000},function(){$('#end_time0').val('');});
				    	}
			        }
				});*/
				...
				//监听提交按钮
              	form.on('submit(submitForm)', function(data){
					...
					var invoice_if = data.field.choose;
        		...
        		//form.on('select(area)', function(data){
    				...
    				// 重置[客户名称]级下面的内容(销售合同编号 金额信息 合同条款)
