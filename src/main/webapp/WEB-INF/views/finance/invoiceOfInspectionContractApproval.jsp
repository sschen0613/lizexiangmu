		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectCurrencyApprover2.action?currency_type=29&currency_string='+staffid;
                    if (date1 != null && date1 != ''){
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            var data = LAY_EXCEL.filterExportData(res.data, {
                                ...
                                currency_int4:function (value,line,data) {
                                    return value == "1" ? "检测费" : "技术服务费";
                                },
                            ...
                            data.unshift({
                                ...
                                currency_string12:"已开票金额",currency_string13:"申请开票金额", currency_int3:"发票类型",currency_int4:"费用类型",
                                currency_string14:"开票事由",currency_int6:"开票信息是否一致"
                            });
                            // 3. 执行导出函数，系统会弹出弹框
                            LAY_EXCEL.exportExcel(data, '信泽检测合同发票开具.xlsx', 'xlsx');
                        }
                    });
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
                            content: 'finance/invoiceOfInspectionContractApprovalDetail.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type
						...
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=29"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '40%'],
							content: 'Currency/approvalOpinion.action?approval_id=29&currency_id='+data.currency_id
						});
					}
				});
