		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectCurrencyApprover2.action?currency_type=70&currency_string='+staffid;
                    if (date1 != null && date1 != ''){
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看
						layer.open({
                            ...
                            content: 'finance/invoiceOfSalesContractTrApprovalDetail.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type
                        });
						...
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=70"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '40%'],
							content: 'Currency/approvalOpinion.action?approval_id=70&currency_id='+data.currency_id
						});
					}
				});
