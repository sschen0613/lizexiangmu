		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectCurrencyApprover2.action?currency_type=23&currency_string='+staffid;
                    if (date1 != null && date1 != ''){
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            data.unshift({
                                ...
                                currency_date2:"运营合同起",currency_date3:"运营合同止",currency_date4:"本次开票起",currency_date5:"本次开票止",
                                currency_string14:"开票事由",currency_int6:"开票信息是否一致"
                            });
                            // 3. 执行导出函数，系统会弹出弹框
                            LAY_EXCEL.exportExcel(data, '丽泽运营合同发票.xlsx', 'xlsx');
                        }
                    });
...
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_date2', title: '运营合同起', minWidth:140, templet:'<div>{{ Format(d.currency_date2,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_date3', title: '运营合同止', minWidth:140, templet:'<div>{{ Format(d.currency_date3,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_date4', title: '本次开票起', minWidth:140, templet:'<div>{{ Format(d.currency_date4,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_date5', title: '本次开票止', minWidth:140, templet:'<div>{{ Format(d.currency_date5,"yyyy-MM-dd")}}</div>'}
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if(layEvent === 'detail'){ //查看
						layer.open({
							...
							maxmin: true,
							area: ['80%', '80%'],
                            content: 'finance/invoiceOfOperationContractApprovalDetail.action?currency_id='+data.currency_id+'&currency_type='+data.currency_type
                        });
						...
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=23"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '50%'],
							content: 'Currency/approvalOpinion.action?approval_id=23&currency_id='+data.currency_id
						});
					}
				});
