	 	...
	 	<form class="layui-form form-top" action="">
	 		<div class="layui-form-item">
	 			<div class="layui-inline">
					<label class="layui-form-label label-revise">请购单号 :</label>
					<div class="layui-input-block">
						<input type="text" name="currency_number" placeholder="请输入请购单" autocomplete="off" class="layui-input input-revise">
					</div>
				</div>
				<div class="layui-inline">
		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectCurrencyApprover2.action?currency_type=30&currency_string='+staffid;
                    if (date1 != null && date1 != ''){
                    ...
                    if (currency_number != null && currency_number != ''){
                        url += '&currency_number='+currency_number;
                    }
                    $.ajax({
                        url: url,
                        dataType: 'json',
                        success: function(res) {
                            var data = LAY_EXCEL.filterExportData(res.data, {
                                ...
                                currency_int6:"currency_int6",
                                currency_string8:"currency_string8",
                                currency_date2:function (value,line,data) {
                                    return Format(value,"yyyy-MM-dd");
                                },
                                currency_string7:"currency_string7",
                                currency_money6:"currency_money6",
                                currency_string5:"currency_string5"
                            });
                            data.unshift({
                                currency_number: '编号',staff_name:"申请人",department_name:"申请部门",currency_date: '申请日期',
                                currency_string4: '客人单位',currency_int6: '客人人数',currency_string8: '陪同人',currency_date2: '招待时间',
                                currency_string7:"招待事由",currency_money6:"申请金额",currency_string5:"申请金额大写"
                            });
                            // 3. 执行导出函数，系统会弹出弹框
                            LAY_EXCEL.exportExcel(data, '业务招待.xlsx', 'xlsx');
                        }
                    });
...
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_number', title: '请购单号', minWidth:200}
						...
						,{field: 'currency_string4', title: '客人单位', minWidth:100}
						,{field: 'currency_int6', title: '客人人数', minWidth:100}
						,{field: 'currency_string8', title: '陪同人', minWidth:80}
						,{field: 'currency_date2', title: '招待时间', minWidth:100, templet:'<div>{{ Format(d.currency_date2,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string7', title: '招待事由', minWidth:100}
						,{field: 'currency_money6', title: '申请金额', minWidth:100}
						,{field: 'currency_string5', title: '申请金额大写', minWidth:120}
					...
					if(layEvent === 'del'){ //删除
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=30"//iframe的url
						}); 
                        ...
                        layer.open({
                            ...
                            maxmin: true,
                            area: ['80%', '80%'],
                            content: 'Currency/businessEntertainmentPrint.action?currency_id='+data.currency_id+'&currency_type='+currency_type
                        });
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '40%'],
							content: 'Currency/approvalOpinion.action?approval_id=30&currency_id='+data.currency_id
						});
					}
				});
