	...
	<div class="layui-form-item">
		<div class="layui-inline">
			...
			<div class="layui-input-block">
				<input type="text" id="currency_number" name="currency_number" placeholder="请输入流程编号" autocomplete="off" class="layui-input input-revise">
			</div>
		</div>
		<div class="layui-inline">
			<label class="layui-form-label label-revise">申请人 :</label>
			<div class="layui-input-block">
				<input type="text" id="currency_string2" name="currency_string2" lay-verify="" placeholder="请输入申请人" autocomplete="off" class="layui-input input-revise">
			</div>
		</div>
		<div class="layui-inline">
		...
		<div class="layui-inline">
			<label class="layui-form-label label-revise">申请类型 :</label>
			<div class="layui-input-block select-revise">
				<select id="type" name="currency_string10" lay-verify="" lay-search class="select-revise">
					<option value="">请选择类型</option>
					<option value="试剂配置">试剂配置</option>
					<option value="标液配制">标液配制</option>
				</select>
			</div>
		</div>
		<div class="layui-inline">
...
<script>
	...
	var page;
	var limit;
	layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form','jquery'], function(){
		...
		var tableInner = table.render({
			elem: '#tab'
			,url: 'Currency/selectCurrencyList.action?currency_type='+ currency_type //数据接口
			,page: true //开启分页
			,toolbar: true
			,title: '试剂标液配置领取审批'
// 			    	,totalRow: true //开启合计行
			,cols: [[ //表头
				...
				,{field: 'currency_number', title: '流程编号', minWidth:200}
				,{field: 'staff_name', title: '申请人', minWidth:80}
				,{field: 'department_name', title: '申请部门', minWidth:100}
				,{field: 'currency_string10', title: '申请类型', minWidth:100}
				,{field: 'currency_date', title: '申请日期', sort: true, minWidth:100, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
				,{field: 'currency_date2', title: '需求日期', sort: true, minWidth:100, templet:'<div>{{ Format(d.currency_date2,"yyyy-MM-dd")}}</div>'}
				//,{field: 'currency_int6', title: '领取确认', minWidth:100}
				//,{field: 'currency_string8', title: '领取人', minWidth:80}
				//,{field: 'currency_date3', title: '领取日期', minWidth:100, templet:'<div>{{ Format(d.currency_date3,"yyyy-MM-dd")}}</div>'}
		...
		$(document).on('click','#ebtn',function () {
			...
			var currency_string2 = $("#currency_string2").val();
			var currency_date2 =$("#date1").val();
			var currency_date3 = $("#date2").val();
			var currency_string10 =  $('#type').find('option:selected').val();
			...
			if(currency_string2 != null && currency_string2 !=''){
				currencyApply += '&currency_string2='+currency_string2;
			}
			...
			if(currency_string10 != null && currency_string10 != ''){
				currencyApply += '&currency_string10='+currency_string10;
			}
			if(currency_number != null && currency_number != ''){
				currencyApply += '&currency_number='+currency_number;
			}
			if(currency_department != null && currency_department != ''){
				currencyApply += '&currency_department='+currency_department;
			}

			window.location.href="Currency/doCurrencyListExcel.action?currency_type="+ currency_type+currencyApply;
		});
		//监听工具条
		table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
			...
			if(layEvent === 'detail'){ //查看
				...
				layer.open({
					...
					maxmin: true,
					area: ['80%', '80%'],
					content: 'Currency/printReagentGet.action?currency_id='+data.currency_id+'&currency_type=22' //iframe的url
				});
				...
				layer.open({
					...
					content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
							+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=22"//iframe的url
				});
				...
				layer.open({
					...
					maxmin: true,
					area: ['30%', '40%'],
					content: 'Currency/approvalOpinion.action?approval_id=22&currency_id='+data.currency_id
				});
			}
		});
