	...
	<body>
	 		...
	 		<div class="layui-form-item">
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input name="currency_date2" type="text" class="layui-input input-revise date-revise" id="date3" placeholder="请选择需求日期">
					</div>
				</div>
				<div class="layui-inline">
					<label class="layui-form-label label-revise">结束日期 :</label>
					<div class="layui-input-block">
						<input name="currency_date3" type="text" class="layui-input input-revise date-revise" id="date4" placeholder="请选择需求日期">
					</div>
				</div>
				<div class="layui-inline">
					<label class="layui-form-label label-revise">请输入流程编号 :</label>
					<div class="layui-input-block">
		...
		<script type="text/html" id="toolbarDemo">
			<div class="layui-btn-container">
				<button class="layui-btn layui-btn-xs" lay-event="add">添加备机请购申请单</button>
			</div>
		</script>
			...
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
				...
				laydate.render({
			        ...
			        ,done: function(value, date, endDate){
				    	...
				    	//开始日选好后，判断结束日期是否合法
				    	if($('#date4').val() != ''){
				    		$('#date4').val() > value ? 1 : layer.msg('请选择正确日期范围',{time: 2000},function(){$('#date4').val('');}); 
				    	}
				    }
				});
				laydate.render({
			        ...
			        ,done: function (value, date, endDate) {
				    	...
				    	//结束日选好后，判断开始日期是否合法
				    	if($('#date3').val() != ''){
				    		$('#date3').val() < value ? 1 : layer.msg('请选择正确日期范围',{time: 2000},function(){$('#date3').val('');});
				    	}
			        }
				});
...
// 				});
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Currency/selectApplicantCurrency.action?currency_type=40' //数据接口
					,page: true //开启分页
					,toolbar: '#toolbarDemo'
			    	,title: '备机请购申请单'
...
// 						,{field: 'proposer', title: '申请人', minWidth:80}	
// 						,{field: 'proposer_department', title: '申请部门', minWidth:100}
						,{field: 'currency_date', title: '申请日期', sort: true, minWidth:100, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
						,{field: 'currency_string4', title: '申请单位', minWidth:100}
						,{field: 'currency_string2', title: '请购区域', minWidth:150}
						,{field: 'currency_date2', title: '需求日期', minWidth:100, sort: true, templet:'<div>{{ Format(d.currency_date,"yyyy-MM-dd")}}</div>'}
						//,{field: '', title: '驾驶员', minWidth:80}
						,{field: 'currency_string7', title: '申请事由', minWidth:200}
						,{field: 'currency_string5', title: '技术要求', minWidth:200}
						,{field: 'currency_string8', title: '其它说明', minWidth:200}
					...
					switch(obj.event){
						case 'add':
							layer.open({
								...
								shadeClose: true,
								area: ['80%', '80%'],
								content: 'onlineOperation/standbyBuyRequestForm.action' //iframe的url
							}); 
				    	break;
				    	case 'delete':
				    		layer.msg('删除');
