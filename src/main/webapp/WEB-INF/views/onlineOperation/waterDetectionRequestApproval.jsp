	 		...
	 		<div class="layui-form-item">
	 			<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input type="text" name="currency_number"  lay-verify="" placeholder="请输入流程编号" autocomplete="off" class="layui-input input-revise">
					</div>
				</div>
				<%--<div class="layui-inline">
		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectCurrencyApprover2.action?currency_type=66&currency_string='+staffid;
                    if (date1 != null && date1 != ''){
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            var data = LAY_EXCEL.filterExportData(res.data, {
                                ...
                                currency_string6:"currency_string6",
                            ...
                            data.unshift({
                                ...
                                currency_string7:"申请事由",
                                details_string2:"企业名称",details_string5:"设备名称",details_string6:"规格型号",details_string8:"服务性质",details_string9:"问题描述",details_string11:"备注"
                            });
                            // 3. 执行导出函数，系统会弹出弹框
                            LAY_EXCEL.exportExcel(data, '技术支持.xlsx', 'xlsx');
                        }
                    });
...
// 			    	,totalRow: true //开启合计行
					,cols: [[ //表头
						...
						,{field: 'currency_number', title: '流程编号', minWidth:100}
                        ...
                        ,{field: 'currency_string2', title: '区域', minWidth:200}
                        ,{field: 'currency_string7', title: '企业名称', minWidth:200}
                        ,{field: 'currency_date4', title: '采样日期', minWidth:200,sort: true, templet:'<div>{{ Format(d.currency_date4,"yyyy-MM-dd")}}</div>'}
                        ,{field: 'currency_date2', title: '计划检测日期', minWidth:200,sort: true, templet:'<div>{{ Format(d.currency_date2,"yyyy-MM-dd")}}</div>'}
                        ,{field: 'currency_date3', title: '计划完成日期', minWidth:200,sort: true, templet:'<div>{{ Format(d.currency_date2,"yyyy-MM-dd")}}</div>'}
					...
					if(layEvent === 'detail'){ //查看
						...
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=66"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '40%'],
							content: 'Currency/approvalOpinion.action?approval_id=66&currency_id='+data.currency_id
						});
					}
				});
