	...
	<body>
		...
		<script type="text/html" id="toolbarDemo">
			<div class="layui-btn-container">
				<button class="layui-btn layui-btn-xs" lay-event="add">添加试剂标液配置领取申请表</button>
				<button class="layui-btn layui-btn-xs layui-btn-danger" lay-event="output">导出试剂标液表</button>
			</div>
		</script>
...
// 				});
				...
				var tableInner = table.render({
					elem: '#tab'
					,url: 'Currency/selectApplicantCurrency.action?currency_type=22' //数据接口
					,page: true //开启分页
					,toolbar: '#toolbarDemo'
			    	,title: '试剂标液配置领取申请表'
					...
					,cols: [[ //表头
						...
						,{field: 'currency_number', title: '流程编号', minWidth:200, sort:true}
						...
						,{field: 'currency_date2', title: '需求时间', minWidth:100, sort: true, templet:'<div>{{Format(d.currency_date2,"yyyy-MM-dd")}}</div>'}
// 						,{field: 'details_string5', title: '试剂/标液名称', minWidth:120}
					...
					switch(obj.event){
						case 'add':
							layer.open({
								...
								shadeClose: true,
								area: ['80%', '80%'],
								content: 'onlineOperation/reagentGetRequestForm.action' //iframe的url
							}); 
				    	break;
				    	case 'output':
                        currencyApply = '';
                        ...
                        var date1 = $('#date1').val();
                        var date2 = $('#date2').val();
                        if(currency_number!==null && currency_number!=='' && currency_number!==undefined){
                            currencyApply += '&currency_number='+currency_number;
                        }
                        if(date1!==null && date1!==''){
                            currencyApply += '&currency_date2='+date1;
                        }
                        if(date2!==null && date2!==''){
                            currencyApply += '&currency_date3='+date2;
                        }
                        window.location.href =
                            'Currency/outputExcel.action?currency_type=22'+currencyApply;
                        break;
				    	case 'delete':
				    		layer.msg('删除');
				    	break;
