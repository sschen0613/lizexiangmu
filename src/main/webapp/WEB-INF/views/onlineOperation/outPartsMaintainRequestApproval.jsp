		...
		//一般直接写在一个js文件中
			layui.use(['laydate', 'layedit', 'laypage', 'layer', 'table',  'element', 'form'], function(){
                ...
                form.on('submit(out)', function(data){
                    ...
                    var url = 'Currency/selectCurrencyApprover2.action?currency_type=61&currency_string='+staffid;
                    if (date1 != null && date1 != ''){
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            data.unshift({
                                ...
                                currency_string7:"申请事由",currency_string4:"故障描述",currency_string5:"申请原因",details_string8:"企业名称",
                                details_string2:"配件名称",details_string6:"规格型号",details_string9:"用途",details_string3:"备注"
                            });
                            // 3. 执行导出函数，系统会弹出弹框
                            LAY_EXCEL.exportExcel(data, '配件内部维修.xlsx', 'xlsx');
                        }
                    });
					...
					if(layEvent === 'detail'){ //查看
						...
						layer.open({
							...
							content: 'Currency/approvalProgress.action?currency_id='+data.currency_id+"&current_approvalCount="
									+data.current_approvalCount+"&approver_count="+data.approver_count+"&approvalOpinion_type=61"//iframe的url
						}); 
						...
						layer.open({
							...
							maxmin: true,
							area: ['30%', '50%'],
							content: 'Currency/approvalOpinion.action?approval_id=61&currency_id='+data.currency_id
						});
					}
				});
